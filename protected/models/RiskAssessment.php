...
class RiskAssessment extends PlantDatabaseActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('type_id, plant_id, maintenance_events_id, service_or_repair_id, site_traffic_control_id', 'required'),
	...
	public function search()
	{
		...
		$criteria=new CDbCriteria;
                ...
                $jobcard_table = JobCard::model()->tableName();
                $jobcard_count_sql = "(select count(*) from $jobcard_table where $jobcard_table.risk_assessment_id = t.id)";

                // select
                $criteria->select = array(
                    '*',
                    $jobcard_count_sql . " as jobcard_count",
                );
                
                // where
                $criteria->compare($jobcard_count_sql, $this->jobcard_count);
		...
		return new CActiveDataProvider($this, array(
			'criteria'=>$criteria,
                            ...
                            'attributes' => array(
                                ...
                                // order by
                                    'jobcard_count' => array(
                                        'asc' => 'jobcard_count ASC',
                                        'desc' => 'jobcard_count DESC',
                                    ),
                                '*',
                                ),
                            ),
